...
- GitHub Actions 기반 CI/CD 배포 워크플로우 구성
...
감독명 검색은 다중 감독 표시와 영화 단위 집계 결과가 직관적으로 유지되도록 포함 검색만 사용합니다.
...
- `.env`: DB 접속 정보 분리
- GitHub Actions: push 후 서버 코드 갱신 및 서비스 재시작
...
- GitHub Actions를 이용해 코드 반영과 서비스 재시작을 자동화하는 방식
